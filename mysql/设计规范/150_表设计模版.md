...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='采购订单主表';
...
CREATE TABLE `bl_po_dtl` (
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_bl_po_dtl_bill_no` (`bill_no`,`material_no`,`size_no`),
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='采购订单明细表';
```
...
如果是 BLF1 项目，请使用 sync_data_sign 字段替换 del_tag 字段作为 otter 数据同步标志：
